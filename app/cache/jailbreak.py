...
- Injected Embedder (shared with semantic cache), not a module-level
  singleton. Makes testing straightforward.
...
Threshold preserved at 0.75 cosine similarity (your v0.1.0 tuning).
"""
...
# Your v0.1.0 blocklist, verbatim. Moved to config default in
# app/config.py — this stays as the module fallback.
...
class JailbreakGuardrail:
    ...
    async def check(self, req: ChatRequest, tenant: Tenant) -> GuardrailResult:
        ...
        # The router and cache both need the request text; we extract
        # once here. text_for_routing returns concatenated content from
        # all messages, which is the right granularity for a
        # jailbreak check (detect injection anywhere in the conversation).
